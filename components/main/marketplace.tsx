          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 sm:gap-8">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="relative h-full flex flex-col">
                <div
                  className="relative h-full flex flex-col bg-[#0b1426]/90 border border-purple-500/10 p-4 sm:p-5"
                  style={{ clipPath: "polygon(0 0,calc(100% - 20px) 0,100% 20px,100% 100%,20px 100%,0 calc(100% - 20px))" }}
                >
                  <div className="absolute top-0 left-0 w-3 h-3 border-t-2 border-l-2 border-purple-500/20 m-2" />
                  <div className="absolute bottom-0 right-0 w-3 h-3 border-b-2 border-r-2 border-green-500/20 m-2" />
                  ...
                  <div className="space-y-2 mb-4 border-l-2 border-purple-500/10 pl-3 flex-grow">
              ...
              <AnimatePresence>
                {visible.map((marketplace, index) => {
                  ...
                  return (
                    ...
                    >
                      <div className="absolute inset-0 bg-gradient-to-br from-purple-500/20 to-green-500/10 group-hover:from-purple-400/35 group-hover:to-green-400/25 blur-md transition-all duration-500" />

                      <div
                        className="relative h-full flex flex-col bg-[#0b1426]/90 backdrop-blur-xl border border-purple-500/30 p-4 sm:p-5 transition-all duration-300 group-hover:border-purple-400 group-hover:shadow-[0_0_20px_rgba(168,85,247,0.2)]"
                        style={{ clipPath: "polygon(0 0, calc(100% - 20px) 0, 100% 20px, 100% 100%, 20px 100%, 0 calc(100% - 20px))" }}
                      >
                        <div className="absolute top-0 left-0 w-3 h-3 border-t-2 border-l-2 border-purple-500/70 m-2 z-10" />
                        <div className="absolute bottom-0 right-0 w-3 h-3 border-b-2 border-r-2 border-green-500/70 m-2 z-10" />

                        {/* Status bar */}
                        <div className="flex items-center justify-between mb-3 border-b border-purple-500/20 pb-2 relative z-10">
                          <span className="text-[10px] uppercase font-mono tracking-widest text-purple-400 flex items-center gap-2">
                        ...
                        {/* Thumbnail */}
                        <div className="relative w-full h-[150px] sm:h-[180px] mb-4 overflow-hidden border border-purple-500/20 group-hover:border-purple-400/50 transition-colors z-10">
                          <img
                          ...
                          {tags.length > 0 && (
                            <div className="flex flex-wrap gap-1.5">
                              {tags.slice(0, 5).map((tag, i) => (
                                <span key={i} className="text-[9px] font-mono uppercase bg-purple-950/40 text-purple-200 px-2 py-0.5 border border-purple-500/20 group-hover:border-purple-500/50">
                                  {tag}
                                </span>
                              ))}
                            </div>
                          ...
                          <div className="flex flex-row items-center gap-2">
                            <Link href={`/marketplace/${marketplace.slug}`} className="flex-1">
                              <button className="w-full px-3 py-1.5 border border-purple-500/40 text-purple-300 font-mono text-[9px] sm:text-[10px] uppercase tracking-widest hover:bg-purple-500/20 hover:border-purple-400 hover:shadow-[0_0_15px_rgba(168,85,247,0.4)] transition-all flex items-center gap-1.5 justify-center relative overflow-hidden group/btn2">
                                ...
                                <svg className="w-2.5 h-2.5 relative z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <Link href={`/marketplace/${marketplace.slug}/checkout`} className="flex-1">
                              <button className="w-full px-3 py-1.5 bg-green-500/10 border border-green-500/40 text-green-300 font-mono font-bold text-[9px] sm:text-[10px] uppercase tracking-widest hover:bg-green-500/30 hover:border-green-400 hover:shadow-[0_0_15px_rgba(34,197,94,0.4)] transition-all flex items-center gap-1.5 justify-center relative overflow-hidden group/btn">
                                <div className="absolute inset-0 w-0 bg-gradient-to-r from-transparent via-green-400/20 to-transparent group-hover/btn:w-full transition-all duration-500 z-0" />
