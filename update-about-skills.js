...
const path = 'components/sub/about-content.tsx';
let content = fs.readFileSync(path, 'utf8');

const educationReplacement = `              {activeTab === "education" && (
                <motion.div
...
const dbSkillsRegex = /<div>\s*<h3 className="flex items-center gap-2 text-\[11px\] text-cyan-300 font-bold uppercase tracking-widest mb-4 border-b border-cyan-500\/30 pb-2">\s*<span className="w-1\.5 h-1\.5 bg-orange-400 rounded-full animate-pulse"><\/span>\s*Database\s*<\/h3>[\s\S]*?<\/div>\s*<div>/g;

let matchResult = content.match(/<div>\s*<h3 className="flex items-center gap-2 text-\[11px\] text-cyan-300 font-bold uppercase tracking-widest mb-4 border-b border-cyan-500\/30 pb-2">\s*<span className="w-1\.5 h-1\.5 bg-orange-400 rounded-full animate-pulse"><\/span>\s*Database\s*<\/h3>[\s\S]*?(?=<div>\s*<h3 className="flex items-center gap-2 text-\[11px\] text-cyan-300)/);

let dbToOSReplacement = `<div>
...
content = content.replace(/{activeTab === "education"[^]*?(?={activeTab === "services")/g, educationReplacement + "\n");
content = content.replace(/<div>\s*<h3 className="flex items-center gap-2 text-\[11px\] text-cyan-300 font-bold uppercase tracking-widest mb-4 border-b border-cyan-500\/30 pb-2">\s*<span className="w-1\.5 h-1\.5 bg-orange-400 rounded-full animate-pulse"><\/span>\s*Database\s*<\/h3>[\s\S]*?(?=<div>\s*<h3 className="flex items-center gap-2 text-\[11px\] text-cyan-300)/, dbToOSReplacement);
